...
def main():
    ...
    # Cuando se escribe el propio board, leer el contenido entrante en lugar del
    # archivo en disco (que aún no existe). Cuando se escribe hypotheses.md, el
    # board debe existir ya en disco.
    if target == "experiment-board.json":
        raw = (payload.get("tool_input", {}) or {}).get("content", "")
        try:
            board = json.loads(raw)
        except (json.JSONDecodeError, ValueError) as exc:
            fail([f"  El contenido de experiment-board.json no es JSON válido: {exc}"])
    else:
